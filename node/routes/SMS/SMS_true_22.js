...
function send_sms() {
  ...
  var allS1 = "";
  console.log("allS1")
  allS1 = hexString.redirect('http://200.123.1.27:80/sendsms?username=admin&password=onelink&phonenumber=0981019963&message=xxx');
...
router.post("/readconfig", function (req, res, next) {
  ...
  // CloseConnection();
  res.render('SMS/asap_send_command_1', { title: 'User List', userData: dataX1, valueimei: controldata1 });
...
router.post("/Send_SMS", function (req, res, next) {
  ...
  var config = {
    method: 'post',
    url: 'http://210.1.9.13:80/sendsms?username=admin&password=onelink&phonenumber=' + setphone + '&message=' + setmsg + '&port=gsm-2.2',
    // url: 'http://200.123.1.27:80/sendsms?username=admin&password=onelink&phonenumber=' + setphone + '&message=' + setmsg + '&port=gsm-2.2',
    headers: {
  ...
  axios(config)
    .then(function (response) {
      ...
      controldata_phone = req.body.dest_num;
      controldata_msg = req.body.msg;
      // console.log('sdsd', dataX1);
      res.render('SMS/SMS_true_22_1', { title: 'User List', userData: dataX1, valueimei: controldata1, value_phone: controldata_phone, value_msg: controldata_msg });
    })
